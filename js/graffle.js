// stolen from http://raphaeljs.com/graffle.html
...
window.onload = function () {
    ...
    for (var i = 0, ii = shapes.length; i < ii; i++) {
        shapes[i].attr({fill: '#666', stroke: '#fff', "fill-opacity": 0, "stroke-width": 2, cursor: "move"});
        shapes[i].drag(move, dragger, up);
        shapes[i].hover(function (event) {
            this.animate({"fill-opacity": 1}, 500);
            this.animate({"stroke-width": 3}, 500);
            this.attr({stroke: '#dd0000'});
        }, function (event) {
            this.animate({"fill-opacity": 0}, 500);
            this.animate({"stroke-width": 2}, 500);
            this.attr({stroke: '#fff'});
        });
        var text = r.text(250, 250, 'foo')  
